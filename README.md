...
- **No video, but site thumb:** if `https://www.worldministries.org/Userfiles/video-thumbs/{YYYYMMDD}.jpg` exists for the schedule air date, show a non-clickable card (same layout as video rows). Override base with `data-thumb-base` on `#content` if needed.
